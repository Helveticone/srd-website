...
// Configuration propre à l'admin Association.
const CONFIG: AdminAccessConfig = {
  teamDomain: process.env.CF_ACCESS_TEAM_DOMAIN ?? '',
  aud: process.env.CF_ACCESS_AUD_ASSOC ?? '',
  role: 'assoc-admin',
};
...
function deny(message: string, status: number) {
  return new NextResponse(`SRD Admin Association — ${message}`, {
    status,
...
export async function middleware(req: NextRequest) {
  ...
  if (
    ...
    process.env.SRD_DEV_ROLE === 'assoc-admin'
  ) {
    return withIdentity(req, { email: 'dev-assoc@srd.ch', role: 'assoc-admin', sub: 'dev' });
  }
